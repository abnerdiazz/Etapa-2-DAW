...
- Vista previa de 4 especialidades del menú, con imagen, nombre, descripción y precio.
...
- `js/inicio-nosotros.js`: sincroniza el contador del carrito y el estado de la cuenta en el navbar de ambas páginas, reutilizando `leerSesion()` de `js/saborexpress-data.js`.
- `css/inicio-nosotros.css`: estilos propios de estas dos vistas (footer, fotos de historia/equipo, imagen del hero). Usa la paleta de colores definida en `css/menu-reportes.css`, por lo que ambas páginas cargan también ese archivo.
- `img/inicio/`: imágenes propias de la página de inicio.
- `img/nosotros/`: imágenes propias de la página institucional.

## Organización del código

`index.html` y `nosotros.html` no cuentan con lógica de negocio propia: son páginas estáticas construidas con HTML5, CSS3 y Bootstrap. El único JavaScript que cargan es `js/inicio-nosotros.js`, un módulo pequeño que solo lee datos ya guardados por otros módulos del equipo (el carrito y la sesión), sin escribir ni modificar esos datos.

El navbar y la paleta de colores (variables `--se-*`) provienen de `css/menu-reportes.css`, compartido con el módulo de Menú y Reportes, de modo que las tres vistas se ven visualmente idénticas. `css/inicio-nosotros.css` solo agrega lo específico de estas dos páginas (footer propio, tamaño de las fotos de la galería y del equipo).
...
| Especialidades | Revisar la sección "Nuestras Especialidades" en Inicio | Se muestran 4 productos con imagen, nombre, descripción y precio. |
...
*(Pendiente: agregar capturas de escritorio y móvil de Inicio y Nosotros en `img/capturas/`, siguiendo el mismo formato que el README de Menú y Reportes.)*

## Ejecución y publicación

Se debe abrir el proyecto desde un servidor HTTP como Live Server, no haciendo doble clic en el HTML, ya que `js/inicio-nosotros.js` se carga como módulo (`type="module"`). Las páginas funcionan como archivos estáticos y no necesitan un proceso de compilación.

Al integrar estos módulos en el despliegue del equipo, deben incluirse `index.html`, `nosotros.html`, `css/inicio-nosotros.css`, `js/inicio-nosotros.js` y las imágenes de `img/inicio/` e `img/nosotros/`. Las rutas son relativas para permitir su uso dentro de un sitio de GitHub Pages.
...
- Estas páginas no consultan ni modifican datos del carrito o la sesión; solo los leen para reflejarlos en el navbar.
